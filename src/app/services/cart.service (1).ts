...
export class CartService {
  ...
  public addToCart(product: Product, quantity: number): CartItem | boolean {
    ...
    console.log("product.selectedSize..quantity",quantity);
    ...
    if(this.get_user_dtls)
    {
      ...
      console.log("get_user_dtls1st",this.get_user_dtls);
      var data:any = [];
      ...
      let hasItem = products.find((items, index) => {
        ...
        // items.selectedSize = product.selectedSize; old change
        ...
        if(items.product.productId == product.productId && oldSize == product.selectedSize) {
          // if(oldSize == product.selectedSize){
            let size =  products[index].selectedSize;
            console.log("New Select Size........AAAAA", size);
            
            let qty = products[index].quantity + quantity;
            let stock = this.calculateStockCounts(products[index], quantity);
            if (qty != 0 && stock) {
              products[index]['quantity'] = qty;
              products[index]['selectedSize'] = size;
              console.log("multiple selectedSize......AAAAA",products[index]['selectedSize']);
  ...
  public updateCartQuantity(product: Product, quantity: number): CartItem | boolean {
    console.log("products",product.selectedSize);
    
    return products.find((items, index) => {
      if(items.product.productId == product.productId && product.selectedSize == products[index].selectedSize) {
       let qty = products[index].quantity + quantity;
        let stock = this.calculateStockCounts(products[index], quantity);
        if (qty != 0 && stock) 
          products[index]['quantity'] = qty;
  ...
  public getCartListData() {
    ...
    this.getCartlistSubscribe = this.http.get(API_URL+this.cartlistapi).subscribe(
      (response: any) => {
        console.log('Cart list12345-----', response);
