    ...
    this.getProductSubscribe = this.http.get(API_URL + this.filter_api).subscribe(
      ...
      (response: any) => {
        ...
        if (this.allproducts.length != 0) {
          ...
          console.log('allproducts2', this.allproducts);

        } else {
          this.allproducts = response.data;
          console.log('allproducts', this.allproducts);
          for (let index = 0; index < this.allproducts.length; index++) {
            if (!this.allproducts[index].slug) {
              let name = this.allproducts[index].productDetails.productName.toLowerCase();
              this.allproducts[index].slug = name.replace(/ /g, "-");
            }
          }
        }
